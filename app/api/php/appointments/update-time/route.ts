...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = hasIndex
      ? `${base}?r=/appointments/update-time&id=${encodeURIComponent(id)}`
      : `${base}/index.php?r=/appointments/update-time&id=${encodeURIComponent(id)}`
    ...
    const res = await fetch(url, { method: 'POST', headers, body: Buffer.from(body) })
